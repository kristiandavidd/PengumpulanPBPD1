...
function hidrateBloodTypes() {
    ...
    x.open("GET", "get_blood_types.php", true);
    x.onreadystatechange = function() {
        if (x.readyState == 4 && x.status == 200) {
            document.getElementById("blood_type_id").innerHTML = x.responseText;
        }
    };
    x.send();
}
...
function hidrateHobbies() {
    var x = getXMLHttpRequest();
    x.open("GET", "get_hobbies.php", true);
    x.onreadystatechange = function() {
        if (x.readyState == 4 && x.status == 200) {
            document.getElementById("hobby_id").innerHTML = x.responseText;
        }
    };
    x.send();
}
...
function validatePhoneNumber(phone_number) {
    var x = getXMLHttpRequest();
    x.open("GET", "get_validate_phone_number.php?phone_number="+phone_number, true);
    console.log(x.responseText);
    x.onreadystatechange = function() {
        if (x.readyState == 4 && x.status == 200) {
            document.getElementById("message_phone_number").innerHTML = x.responseText;
        }
    };
    x.send();
}
