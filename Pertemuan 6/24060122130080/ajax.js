...
function hidrateBloodTypes() {
    $.ajax({
        url: 'get_blood_types.php',
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            let bloodTypeSelect = $('#blood_type');
            bloodTypeSelect.empty();
            response.forEach(function(bloodType) {
                bloodTypeSelect.append(new Option(bloodType.name, bloodType.id));
            });
        },
...
function hidrateHobbies() {
    $.ajax({
        ...
        dataType: 'json',
        success: function(response) {
            let hobbySelect = $('#hobby');
            hobbySelect.empty();
            response.forEach(function(hobby) {
                hobbySelect.append(new Option(hobby.name, hobby.id));
            });
        },
        error: function(xhr, status, error) {
...
function validatePhoneNumber(phone) {
    $.ajax({
        url: 'get_validate_phone_number.php',
        type: 'POST',
        data: { phone: phone },
        dataType: 'json',
        success: function(response) {
